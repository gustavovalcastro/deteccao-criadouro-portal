...
(function () {
  ...
  const Campaigns = {
    ...
    async load() {
      ...
      try {
        ...
        if (!rows || !rows.length) {
          ...
          tbody.innerHTML = `<tr><td colspan="4" class="muted">Nenhuma campanha</td></tr>`;
          return;
        }
            ...
            return `
              <tr>
                ...
                <td class="text-right">
                  <a href="#/campaigns/${
                    c.id
                  }" class="btn small" onclick="event.stopPropagation()">Editar</a>
                  <button class="btn small danger" onclick="event.stopPropagation(); Campaigns.remove(${
                    c.id
                  })">Excluir</button>
                </td>
              </tr>`;
        ...
        console.error(error);
        tbody.innerHTML = `<tr><td colspan="4" class="muted">Erro ao carregar campanhas.</td></tr>`;
      }
    },
    ...
    openDetails(id) {
      ...
      if (oriEl) oriEl.textContent = oriArr.length ? oriArr.join("; ") : "—";
  ...
  const EditCampaign = {
    ...
    async load(id) {
      ...
      try {
        ...
        console.error(error);
        root.innerHTML = `<div class="card p24">Erro ao carregar campanha.</div>`;
      }
    },
  ...
  // DETALHES DO RESULTADO: exibe informações completas de um resultado
  const ResultDetails = {
    render(id) {
      if (!h.requireAuth()) return "";
      // casca com placeholder; depois o load() preenche
      return h.shell(`
        <div id="result-details-root">
          <div class="card p24">Carregando resultado...</div>
        </div>
      `);
    },

    async load(id) {
      const root = document.getElementById("result-details-root");
      if (!root) return;
      ...
      try {
        ...
        if (!result) {
          root.innerHTML = `<div class="card p24">Resultado não encontrado.</div>`;
          return;
        }
        ...
        const tipo = result.type === "propriedade" ? "Residência" : "Terreno";
        const tipoClass = result.type === "propriedade" ? "blue" : "orange";
        const statusText =
          result.status === "finished"
            ? "Finalizado"
            : result.status === "processing"
            ? "Processando"
            : result.status === "visualized"
            ? "Visualizado"
        ...
        <div class="card p24">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
            <h2 style="margin:0">Detalhes do Resultado</h2>
            <a href="#/map" class="btn">Voltar ao Mapa</a>
          </div>
          ...
          <div class="row cols-2 mt16">
            <div>
              ...
              <div class="mt4">
                <span class="chip ${tipoClass}">${tipo}</span>
              </div>
            </div>
            <div>
              <div class="label"><b>Status</b></div>
              <div class="mt4 muted">${statusText}</div>
            </div>
          </div>

          <div class="row cols-2 mt16">
            <div>
              <div class="label"><b>ID do Resultado</b></div>
              <div class="mt4 muted">${result.id}</div>
            </div>
            <div>
              <div class="label"><b>Objetos Detectados</b></div>
              <div class="mt4 muted">${result.object_count || 0}</div>
            </div>
          </div>

          <div class="row cols-2 mt16">
            <div>
              <div class="label"><b>Data de Criação</b></div>
              <div class="mt4 muted">${formatDateTime(result.created_at)}</div>
            </div>
            <div>
              <div class="label"><b>Data de Processamento</b></div>
              <div class="mt4 muted">${formatDateTime(
                result.processed_at
              )}</div>
            </div>
          </div>
          ...
          <div class="mt16">
            <div class="label"><b>Coordenadas</b></div>
            <div class="mt4 muted">Lat: ${lat}, Lng: ${lng}</div>
          </div>
          ...
          ${
            result.campaignId
              ? `
          <div class="mt16">
            <div class="label"><b>Campanha</b></div>
            <div class="mt4">
              <a href="#/campaigns/${result.campaignId}" class="muted">ID: ${result.campaignId}</a>
            </div>
          </div>
          `
              : ""
          ...
          <div class="mt16">
            <div class="label"><b>Feedback</b></div>
            <div class="mt4 muted">
              ${
                result.feedback?.like !== null
                  ? result.feedback.like
                    ? "👍 Curtiu"
                    : "👎 Não curtiu"
                  : "Sem feedback"
              }
              ${
                result.feedback?.comment
                  ? `<br>Comentário: ${escapeHtml(result.feedback.comment)}`
                  : ""
              }
            </div>
          </div>
          ...
          <div class="mt24">
            <div class="label"><b>Imagem Original</b></div>
            <div class="mt8">
              ${
                result.originalImage
                  ? `<img src="${escapeHtml(
                      result.originalImage
                    )}" alt="Imagem original" style="max-width:100%;border-radius:8px;border:1px solid #eee" />`
                  : '<div class="muted">Sem imagem original</div>'
              }
            </div>
          </div>

          <div class="mt24">
            <div class="label"><b>Imagem Processada</b></div>
            <div class="mt8">
              ${
                result.resultImage
                  ? `<img src="${escapeHtml(
                      result.resultImage
                    )}" alt="Imagem processada" style="max-width:100%;border-radius:8px;border:1px solid #eee" />`
                  : '<div class="muted">Sem imagem processada</div>'
              }
            </div>
          </div>
        </div>
        ...
        console.error(error);
        root.innerHTML = `<div class="card p24">Erro ao carregar resultado.</div>`;
      }
    },
  };
  ...
  function router() {
    ...
    if (r.startsWith("#/login")) app.innerHTML = Login.render();
      ...
      const id = r.split("/")[2];
      app.innerHTML = ResultDetails.render(id);
      ResultDetails.load(id);
  ...
  window.ResultDetails = ResultDetails;
