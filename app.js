...
(function () {
  ...
  const Campaigns = {
        ...
        <div class="modal" id="modal-campaign">
          <div class="box">
            ...
            <form onsubmit="return Campaigns.create(event)">
              ...
              <textarea class="input" id="c-desc"></textarea>

              <div class="row cols-2 mt12">
                <div>
                  <label class="label">Início</label>
                  <input type="date" className="input" id="c-start" />
                </div>
                <div>
                  <label class="label">Fim</label>
                  <input type="date" className="input" id="c-end" />
                </div>
              </div>
              ...
              <div class="mt12"><label><input type="checkbox" id="c-active" checked/> Ativa</label></div>
    ...
    openModal(){ document.getElementById('modal-campaign').classList.add('show'); },
    ...
    create(e){
      ...
      const payload = {
        ...
        period: { start: document.getElementById('c-start').value, end: document.getElementById('c-end').value },
        is_active: document.getElementById('c-active').checked,
        // Campos como listas (separadas por ;) para facilitar edição
        campaignInfos: (document.getElementById('c-infos').value || '').split(';').map(s=>s.trim()).filter(Boolean),
        instructionInfos: (document.getElementById('c-ori').value || '').split(';').map(s=>s.trim()).filter(Boolean),
      };
      Store.createCampaign(payload);
      Campaigns.closeModal();
      location.reload(); // recarrega a lista para ver a nova campanha
      return false;
    }
  };
  ...
  const EditCampaign = {
    render(id){
      ...
      return h.shell(`
        <form class="card p24" onsubmit="return EditCampaign.save(event, ${c.id})">
          ...
          <label class="label">Nome</label>
          <input class="input" id="e-title" value="${escapeHtml(c.title)}" />
          <label class="label mt12">Descrição</label>
          <textarea class="input" id="e-desc">${escapeHtml(c.description||'')}</textarea>

          <div class="row cols-2 mt12">
            <div>
              <label class="label">Início</label>
              <input type="date" class="input" id="e-start" value="${c.period?.start||''}" />
            </div>
            <div>
              <label class="label">Fim</label>
              <input type="date" class="input" id="e-end" value="${c.period?.end||''}" />
            </div>
          </div>
          ...
          <div class="mt12"><label><input type="checkbox" id="e-active" ${c.is_active?'checked':''}/> Ativa</label></div>
    ...
    save(e, id){
      ...
      Store.updateCampaign(id, {
        ...
        period: { start: document.getElementById('e-start').value, end: document.getElementById('e-end').value },
        is_active: document.getElementById('e-active').checked,
        campaignInfos: (document.getElementById('e-infos').value || '').split(';').map(s=>s.trim()).filter(Boolean),
        instructionInfos: (document.getElementById('e-ori').value || '').split(';').map(s=>s.trim()).filter(Boolean),
      });
