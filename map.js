...
const MapView = (function () {
  ...
  function init(containerId, data = [], opts = {}) {
    ...
    if (points.length) {
      ...
      // 🔹 NÃO deixar o mapa dar zoom demais em um ponto só
      // usa cfg.zoom como zoom máximo (padrão 13)
      const maxZoom = cfg.zoom || 13;
    ...
    return `
      <div style="max-width:220px">
        ...
        <div style="margin-top:8px">
          <a href="#/result/${p.id}" style="color:#2563eb;text-decoration:underline;font-size:13px">Ver detalhes</a>
        </div>
      </div>`;
  }
  ...
  function drawGrid(stepDeg, points, panel) {
    ...
    for (let lat = startLat; lat < ne.lat(); lat += stepDeg) {
      for (let lng = startLng; lng < ne.lng(); lng += stepDeg) {
        ...
        // contagens
        const countResid = inside.filter(
          (p) => p.type === "propriedade"
        ).length;
        ...
        rect.addListener("click", () => {
          ...
          overlays.forEach((o) => {
            if (o instanceof google.maps.Rectangle)
              o.setOptions({ fillOpacity: 0.06 });
          });
  ...
  function rowHtml(p) {
    ...
    const data = p.created_at
      ...
      : "—";
    return `
      <div class="panel-item" onclick="location.hash='#/result/${
        p.id
  ...
  function ensureData(data, center) {
    // Retorna apenas os dados reais da API, sem gerar dados mockados
    if (Array.isArray(data) && data.length) {
      return data.map(normalizePoint);
    }
    // Se não houver dados, retorna array vazio (não gera dados simulados)
    return [];
  }
