...
(function () {
  ...
  function createUser({name, email, password, city}){
    ...
    const maxId = s.users.reduce((m,u)=>Math.max(m, Number(u.id)||0), 0);
    s.users.push({
        id: maxId + 1,
  ...
  function deleteUser(id){
    ...
    const i = s.users.findIndex(u=>u.id == id);
    if (i === -1) throw new Error('Usuário não encontrado');
    if (s.users[i].email === 'teste@gmail.com') throw new Error('Não é permitido apagar o gestor padrão');
    s.users.splice(i,1);
    setStore(s);
  }
  ...
  const AdminPanel = {
    ...
    deleteUser(id){
        ...
        // PASSO 5 — proteção: não permitir excluir o gestor padrão
        ...
        try{
            // usa a função helper de armazenamento (a que já existe acima no arquivo)
            deleteUser(id);
            location.hash = '#/admin';   // recarrega a lista
        }catch(ex){
