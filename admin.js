...
(function () {
  ...
  function updateUser(id, patch){
    ...
    if ('name' in patch && !String(patch.name).trim())  throw new Error('Nome é obrigatório');
    if ('city' in patch && !String(patch.city).trim())  throw new Error('Cidade é obrigatória');
  ...
  const AdminPanel = {
    ...
    saveEdit(e){
      ...
      try{
        ...
        updateUser(id, patch); // helper que salva no localStorage
        ...
        setTimeout(()=>{
          ...
          if (typeof AdminPanel.refresh === 'function') {
            AdminPanel.refresh();
