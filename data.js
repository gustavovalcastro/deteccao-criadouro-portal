...
(function (w) {
  ...
  const Store = {
    ...
    async listDetections(campaignId) {
      const user = this.currentUser();
      if (!user || !user.city) {
        return [];
      }
      ...
      try {
        // Busca detecções da cidade do usuário logado
        const results = await API.post("/results/getResultByCity", {
          city: user.city,
        });

        // Verifica se a resposta é um array
        if (!Array.isArray(results)) {
          console.warn("Resposta da API não é um array:", results);
          return [];
        }

        // Normaliza os dados da API para o formato esperado pelo mapa
        let normalized = results
          .map((item) => {
            // Valida que coordinates existe e tem lat/lng válidos
            if (
              !item.coordinates ||
            ...
            // Converte coordinates.lat e coordinates.lng (strings) para lat e lng (numbers)
            ...
            return {
              id: Number(item.id) || item.id, // Garante que o ID seja número se possível
              lat,
              lng,
              type: item.type || "terreno",
          ...
          .filter(Boolean); // Remove itens inválidos (null)

        // Aplica filtro por campaignId se fornecido
        if (campaignId) {
          normalized = normalized.filter(
            (d) => String(d.campaignId) === String(campaignId)
          );
        }
    ...
    async getResult(id) {
      ...
      try {
        const result = await API.get(`/results/getResult/${id}`);
        ...
        const normalized = {
          ...
          feedback: result.feedback || { like: null, comment: null },
          coordinates: result.coordinates
            ? {
                lat: Number(result.coordinates.lat) || 0,
                lng: Number(result.coordinates.lng) || 0,
              }
            : null,
        };
